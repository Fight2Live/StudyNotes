...
        当线程执行一个方法时，就会随之创建一个对应的栈帧，并将建立的栈帧压栈。当方法执行完毕之后便会将栈帧出栈
...
        所有新生成的对象都会首先落在新生代，而新生代的目标是尽可能快速的收集掉那些生命周期短的对象。
...
        在新生代中经历了N次回收后仍然存活的对象，会被放到老年代中。因此可以认为老年代中存放的都是一些生命周期较长的对象。
...
        在使用默认参数时，绝大部分情况下都是不需要调优的，但也会有少部分场景需要调整。

        整体上可以分为几个流程

1. 监控/查看相关指标，观察是否存在异常或瓶颈
   ...
   1. CPU
      ...
      > 常用工具：JProfiler, JVM Profiler, Arhas
   ...
   3. GC
      ...
      - 单次GC各阶段详细耗时，找到耗时严重的阶段
      
      - 对象的动态晋升年龄是否正常

2. 确定优化目标，如：
   
   - 将FGC次数从每小时1次，降低到三小时1次
...
3. 制定优化方案，常见的有：
   ...
   - 不合理的JVM参数配置：年轻代内存配置过小、堆内存配置过小、元空间配置过小等
